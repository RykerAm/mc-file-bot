...
# Supabase Connection String (External URL)
...
async def sms_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ ပုံစံ - /Sms @username စာသား")
...
async def give_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.close(); conn.close()
    for r in dbf:
        found.append((r[0], r[1]))
    if found:
        await update.message.reply_document(found[0][1], caption=found[0][0])
...
async def btn_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    if q.data.startswith("ul_"):
        await user_list(update, context)
    elif q.data == "main":
        await list_menu(update, context)
    elif q.data.startswith("cat_"):
        cn = q.data.replace("cat_", "")
        res = f"<b>📂 {cn}</b>\n\n"
        for f in file_database.get(cn, {}).keys():
            res += f"• <code>{f}</code>\n"
        conn = get_db_conn(); cur = conn.cursor()
