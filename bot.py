...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton("Next", callback_data=f"sp_{page+1}")]] if len(u_ids) > 30 else []
...
1️⃣ **File ကိုတိုက်ရိုက်ရှာရန်:** Bot ရဲ့ Chat ထဲမှာ ဖိုင်နာမည်ကို ရိုက်ပို့လိုက်ရုံပါပဲ။
   _ဥပမာ- `One Piece Addon` သို့မဟုတ် `one piece`_
...
_ဥပမာ- `/give Naruto Addon`_

---

**၃။ Bot ဆီမှာ မတင်ရသေးသော file တင်ခိုင်းရန် နှင့် Feedback**

* **Bot ဆီမှာမရှိသေးတဲ့ file တင်ခိုင်းရန်:** `/req [ဖိုင်နာမည်]`
...
async def btn_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith("sp_"):
        ...
        kb = []
        nav = []
        if page > 0: nav.append(InlineKeyboardButton("Back", callback_data=f"sp_{page-1}"))
        if (page+1)*30 < len(u_ids): nav.append(InlineKeyboardButton("Next", callback_data=f"sp_{page+1}"))
...
async def req_fb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg: return
    label = "Request" if update.message.text.startswith("/req") else "Feedback"
